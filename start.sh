...
echo "Updating /etc/hosts to make hadoop-docker point to $docker_ip ($docker_hostname)"
if grep 'hadoop-docker' /etc/hosts >/dev/null; then
  sudo sed -i "s/^.*hadoop-docker.*\$/$docker_ip hadoop-docker $docker_hostname/" /etc/hosts
else
  sudo sh -c "echo '$docker_ip hadoop-docker $docker_hostname' >> /etc/hosts"
fi
